...
def get_apk_info(file_path):
    try:
        ...
        if (system_name != 'Darwin' and system_name != 'Linux' and system_name != 'Windows'):
            raise TypeError('unknown system type, only support Darwin、Linux、Windows')
        aapt_path = os.path.join(os.getcwd(), 'bin', system_name)
        out = subprocess.getoutput('cp ' + file_path + ' ' + aapt_path +'/')
        file_path = os.path.join(aapt_path, file_path.split('/')[-1])
        stdout = dump(file_path, 'badging')
        out = subprocess.getoutput('rm -rf  ' + file_path )
        ...
        match = re.compile("launchable-activity: name='([a-zA-Z0-9-\\S.]+)'").search(stdout)
        try:
            app_activity = match.group(1)
